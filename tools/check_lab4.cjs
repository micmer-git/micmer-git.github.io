/* Smoke test jsdom per top-20/lab4.html — 50 varianti in 5 temi (round 4, 2026-08-05).
 *
...
const html = fs.readFileSync(path.join(ROOT, "top-20", "lab4.html"), "utf8");
...
if (cards.length !== 51) errors.push("schede: " + cards.length + " (attese 51 = base + 50)");
const votable = d.querySelectorAll(".stbtn");
if (votable.length !== 50) errors.push("stelle: " + votable.length + " (attese 50)");
const chips = d.querySelectorAll(".chip");
if (chips.length !== 550) errors.push("chip: " + chips.length + " (attesi 550 = 50 × 11)");
const navs = d.querySelectorAll("#tnav a");
if (navs.length !== 5) errors.push("temi in nav: " + navs.length + " (attesi 5)");
const fams = d.querySelectorAll("section.fam");
if (fams.length !== 6) errors.push("sezioni: " + fams.length + " (attese 6)");
...
/* --- voto, stella, blob --- */
const c1 = d.getElementById("tr01");
...
if (!/1\s*\/\s*50/.test(prog.replace(/\s+/g, " "))) errors.push("progresso: «" + prog + "»");
const navB = d.querySelector('#tnav b[data-fam="transizione"]');
...
if (!/VOTI LAB4/.test(blob)) errors.push("blob senza intestazione");
if (!/★ TR01/.test(blob)) errors.push("blob senza la preferita");
if (!/PREFERITE: TR01/.test(blob)) errors.push("blob senza il riepilogo preferite");

if (errors.length) { console.error("✗ lab4\n- " + errors.join("\n- ")); process.exit(1); }
console.log("✓ lab4: 51 schede, 5 temi × 10, " + Math.round(ts / 1000) + "s di animazione simulata, voto e blob ok");
